...
def output_labels(labels, row_label, prefix=''):
    ...
    for l in range(ns):
        ...
        f.write(listrl[l] + ',' + ','.join((list(map(str, list(labels[l, :]))))) + '\n')
    f.close()
...
def plot_top_relationships(data, mis, column_label, cont, topk=5, prefix=''):
    ...
    for j in range(m):
        ...
        if len(inds) >= 2:
            plot_rels(data[:, inds], [column_label[q] for q in inds], colors=cont[:, j],
...
def vis_hierarchy(sieve, column_label, max_edges=200, prefix=''):
    ...
    import textwrap
    column_label = ['\n'.join(textwrap.wrap(q, width=20)) for q in column_label]
...
def edge2pdf(g, filename, threshold=0, position=None, labels=None, connected=True, directed=False, makepdf=True):
    ...
    for n in g.nodes():
        if labels is not None:
            if type(labels) == dict or type(labels) == list:
                ...
                if 'label' in g._node[n]:
                    thislabel = g._node[n][labels].replace('"', '\\"')
                    # combine dupes
                    ...
                    if n[0] == 1:
                        lstring = 'shape=circle,margin="0,0",style=filled,fillcolor=black,fontcolor=white,height=%0.2f,label="Y%d"' % (
                            2 * weight, n[1])
                    else:
        ...
        f.write(('\t' + cnn(n) + finalstring).encode('utf-8'))
